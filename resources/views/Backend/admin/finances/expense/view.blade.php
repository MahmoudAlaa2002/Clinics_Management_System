                        ...
                        <tbody id="appointments_table_body">
                            @if($expenses->count() > 0)
                                @foreach ($expenses as $expense)
                                    <tr>
                                        <td>{{ $expense->id }}</td>
                                        <td>{{ $expense->vendor_invoice_id }}</td>
                                        <td>{{ $expense->vendorInvoice->clinic->name }}</td>
                                        <td>${{ $expense->vendorInvoice->final_amount }}</td>
                                        <td class="action-btns">
                                            <div class="d-flex justify-content-center">
                                                <a href="{{ route('details_expense', ['id' => $expense->id]) }}" class="mr-1 btn btn-outline-success btn-sm"><i class="fa fa-eye"></i></a>
                                                <a href="{{ route('edit_expense', ['id' => $expense->id]) }}" class="mr-1 btn btn-outline-primary btn-sm"><i class="fa fa-edit"></i></a>
                                                <button class="btn btn-outline-danger btn-sm delete-expense" data-id="{{ $expense->id }}"><i class="fa fa-trash"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
