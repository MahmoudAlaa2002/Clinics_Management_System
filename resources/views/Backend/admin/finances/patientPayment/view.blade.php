...
@section('title' , 'View Patients Payments')
    ...
    <div class="content">
        <div class="row">
            <div class="col-sm-4 col-3">
                <h4 class="page-title">View Patients Payments</h4>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                                    ...
                                    <tr>
                                        ...
                                        <td class="action-btns">
                                            <div class="d-flex justify-content-center">
                                                <a href="{{ route('details_payment', ['id' => $payment->id]) }}" class="mr-1 btn btn-outline-success btn-sm"><i class="fa fa-eye"></i></a>
                                                <a href="{{ route('edit_payment', ['id' => $payment->id]) }}" class="mr-1 btn btn-outline-primary btn-sm"><i class="fa fa-edit"></i></a>
                                                <button class="btn btn-outline-danger btn-sm delete-payment" data-id="{{ $payment->id }}"><i class="fa fa-trash"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                                ...
                                <tr>
                                    <td colspan="9" class="text-center">
                                        <div  style="font-weight: bold; font-size: 18px; margin-top:15px;">
                                            No patients payments available at the moment
                                        </div>
                                    </td>
                                </tr>
                            @endif
...
<script>
    $(document).on('click', '.delete-payment', function () {
        let patientPaymentId = $(this).data('id');
        let url = `/admin/delete/payment/${patientPaymentId}`;
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            Swal.fire({
                                title: 'Deleted!',
                                text: 'Patient Payment Has Been Deleted Successfully',
                                icon: 'success'
                            }).then(() => {
                                location.reload();
                            });
