...
@section('title' , 'View Vendors Invoices')
    ...
    <div class="content">
        <div class="row">
            <div class="col-sm-4 col-3">
                <h4 class="page-title">View Vendors Invoices</h4>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive">
                            ...
                            @if($vendorInvoices->count() > 0)
                                @foreach ($vendorInvoices as $vendorInvoice)
                                    <tr>
                                        ...
                                        <td>{{ $vendorInvoice->vendor->name }}</td>
                                        <td>{{ $vendorInvoice->clinic->name }}</td>
                                        <td>{{ $vendorInvoice->invoice_date }}</td>
                                        <td>${{ $vendorInvoice->final_amount }}</td>
                                        <td>
                                            @php
                                                $amount_paid = $vendorInvoice->expenses->sum(function($expense) {
                                                    return $expense->expenseItems->sum('total_amount');
                                                });
                                            @endphp
                                            ...
                                            @elseif($amount_paid < $vendorInvoice->final_amount)
                                        ...
                                        <td class="action-btns">
                                            <div class="d-flex justify-content-center">
                                                <a href="{{ route('details_vendor_invoice', ['id' => $vendorInvoice->id]) }}" class="mr-1 btn btn-outline-success btn-sm"><i class="fa fa-eye"></i></a>
                                                <a href="{{ route('edit_vendor_invoice', ['id' => $vendorInvoice->id]) }}" class="mr-1 btn btn-outline-primary btn-sm"><i class="fa fa-edit"></i></a>
                                                <button class="btn btn-outline-danger btn-sm delete-vendorInvoice" data-id="{{ $vendorInvoice->id }}"><i class="fa fa-trash"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                                ...
                                <tr>
                                    <td colspan="9" class="text-center">
                                        <div  style="font-weight: bold; font-size: 18px; margin-top:15px;">
                                            No Vendors Invoices Available At The Moment
                                        </div>
                                    </td>
                                </tr>
                            @endif
                    ...
                    <div id="main-pagination" class="pagination-wrapper d-flex justify-content-center">
                        {{ $vendorInvoices->links('pagination::bootstrap-4') }}
                    </div>
                </div>
            </div>
        </div>
