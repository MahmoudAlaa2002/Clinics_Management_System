        ...
        <div class="card-box">
            <form action="{{ route('doctor.medical_records.store') }}" method="POST">
                ...
                <div class="form-group mb-3">
                    <label class="form-label"><i class="fa fa-calendar-check text-primary me-2"></i> Select Appointment</label>
                    <select name="appointment_id" id="appointment_id" class="form-control" required>
                        ...
                        @foreach($appointments as $appointment)
                            <option value="{{ $appointment->id }}" data-patient="{{ $appointment->patient->id }}">
                                {{ $appointment->patient->user->name }} — {{ \Carbon\Carbon::parse($appointment->date)->format('Y-m-d') }} {{ \Carbon\Carbon::parse($appointment->time)->format('H:i A') }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                {{-- Prescriptions --}}
                <div class="form-group mb-3">
                    <label class="form-label"><i class="fa fa-prescription-bottle-alt text-primary me-2"></i> Prescriptions</label>
                ...
                {{-- Attachments --}}
                <div class="form-group mb-3">
                    <label class="form-label"><i class="fa fa-paperclip text-primary me-2"></i> Attachments (optional)</label>
                ...
                <div class="d-flex justify-content-between">
                    <a href="{{ route('doctor.medical_records') }}" class="btn btn-outline-secondary btn-rounded">
