        ...
        <div class="table-responsive">
            <table class="table mb-0 text-center table-bordered table-striped custom-table">
                ...
                <tbody id="medical_records_table_body">
                    @include('Backend.clinics_managers.medical_records.search', ['medical_records' => $medical_records])
                </tbody>
            </table>
...
<script>
    $(document).on('click', '.delete-medical-record', function () {
        ...
        let url = `/clinic-manager/delete/medical-record/${medicalRecordsId}`;
    ...
    function fetchMedicalRecords(url = "{{ route('clinic.search_medical_records') }}") {
        ...
        if (keyword === '' && lastKeyword !== '') {
            lastKeyword = '';
            window.location.href = "{{ route('clinic.view_medical_records') }}";
            return;
        }
