                                        ...
                                        @switch($key)
                                            ...
                                            @case('new_invoice')
                                                @if(($data['payment_status'] ?? '') === 'Paid')
                                                    A new
                                                    <strong>fully paid invoice</strong> has been created for
                                                    <strong>{{ $data['patient_name'] ?? 'the patient' }}</strong>.

                                                @elseif(($data['payment_status'] ?? '') === 'Partially Paid')
                                                    A new
                                                    <strong>partially paid invoice</strong> has been created for
                                                    <strong>{{ $data['patient_name'] ?? 'the patient' }}</strong>.

                                                @else
                                                    A new
                                                    <strong>unpaid invoice</strong> has been created for
                                                    <strong>{{ $data['patient_name'] ?? 'the patient' }}</strong>.
                                                @endif
                                                @break
                                            ...
                                            {{-- لما موظف الاستقبال يحجز للمريض مع نفس الدكتور --}}
                                            @case('appointment_booked_by_receptionist')
                                                An appointment with you has been booked for
                                                <strong>{{ $data['patient_name'] ?? '' }}</strong>
                                                by the department receptionist
