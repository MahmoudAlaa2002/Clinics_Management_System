                    ...
                    <div class="col-md-4 col-sm-4 col-lg-3">
                        <div class="profile-widget">
                            <div class="doctor-img">
                                <a class="avatar" href="{{ Route('nurse.profile_doctor' , ['id' => $doctor->id]) }}"> <img src="{{ optional(optional($doctor->employee)->user)->image
                                    ? asset(optional($doctor->employee->user)->image)
                                    : asset('assets/img/user.jpg') }}"></a>
                            </div>

                            <h4 class="doctor-name text-ellipsis" style="margin-bottom: 7px;"><a href="{{ Route('nurse.profile_doctor' , ['id' => $doctor->id]) }}">{{ $doctor->employee->user->name }}</a></h4>
...
<script>
    ...
    function fetchDoctors(url = "{{ route('nurse.search_doctors') }}") {
        ...
        if (keyword === '' && lastDoctorKeyword !== '') {
            lastDoctorKeyword = '';
            window.location.href = "{{ route('nurse.view_doctors') }}";
            return;
        }
